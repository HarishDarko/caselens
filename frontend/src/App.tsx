...
function App() {
  ...
  async function loadScenario(scenarioKey: string) {
    setBusy(true); setError(null)
    try { const scenario = mapApiTicket(await api.createScenario(scenarioKey)); setTickets((current) => current.some((ticket) => ticket.backendId === scenario.backendId) ? current : [...current, scenario]); setSelectedTicketId(scenario.id); setDraftUrgency(null); setFeedbackNote(''); setFeedbackSaved(false); setView('inbox'); await hydrateTriage(scenario) }
...
function InboxView({ tickets, selectedTicket, draftUrgency, feedbackNote, feedbackSaved, busy, onLoadScenario, onSelectTicket, onBack, onDraftUrgency, onFeedbackNote, onSaveFeedback }: InboxViewProps) {
  return <div className="page-content"><div className="page-heading"><div><p className="eyebrow">Human review queue</p><h1>Ticket inbox</h1><p className="page-intro">Prioritized cases with the evidence behind each recommendation.</p></div><div className="heading-meta"><span className="metric-label">Queue health</span><strong><span className="status-dot" />{busy ? 'Processing' : 'Stable'}</strong><span className="metric-detail">{tickets.length} synthetic cases</span></div></div><div className={selectedTicket ? 'inbox-layout with-detail' : 'inbox-layout'}><section className="queue-column" aria-labelledby="queue-heading"><div className="section-bar"><div><h2 id="queue-heading">Needs review</h2><span className="muted">Sorted by priority and SLA risk</span></div><span className="count-pill">{tickets.length}</span></div><div className="scenario-card"><div><span className="scenario-kicker">Try a scenario</span><strong>Site-wide availability signal</strong><span className="muted">Create a synthetic incident and send it through the real queue worker.</span></div><button className="secondary-button" onClick={() => void onLoadScenario('charger-offline-site-wide')} disabled={busy}>Load charger-offline-site-wide</button><div className="scenario-divider" /><div><span className="scenario-kicker">Failure handling</span><strong>Controlled retry demonstration</strong><span className="muted">Records one synthetic provider timeout, then exposes the job in Operations for a reviewer retry.</span></div><button className="secondary-button retry-demo-button" onClick={() => void onLoadScenario('provider-retry-demo')} disabled={busy}>Run controlled retry demo</button></div><div className="ticket-list">{tickets.map((ticket) => <TicketRow key={`${ticket.id}-${ticket.backendId ?? 'fixture'}`} ticket={ticket} selected={ticket.id === selectedTicket?.id} onSelect={onSelectTicket} />)}</div></section>{selectedTicket && <TicketDetail ticket={selectedTicket} draftUrgency={draftUrgency} feedbackNote={feedbackNote} feedbackSaved={feedbackSaved} busy={busy} onBack={onBack} onDraftUrgency={onDraftUrgency} onFeedbackNote={onFeedbackNote} onSaveFeedback={onSaveFeedback} />}</div></div>
}
