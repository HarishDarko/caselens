...
import { createCaseLensApi, type ApiTicket, type EvaluationSummary, type FailurePage, type TriageProcessing, type TriageResult } from './api'
...
type Ticket = {
  id: string; backendId: string; subject: string; message: string; category: string; urgency: Urgency | null; priority: number | null; reliabilitySignal: string | null; slaRisk: string; source: string; summary: string; rationale: string; evidence: string[]; actions: string[]; suggestedReply: string; policy: string; timeline: TimelineEvent[]; triageResultId: string | null; decisionSource: string | null; triageStatus: string; scenarioKey: string | null; latencyMs: number | null
}
...
function App() {
  ...
  useEffect(() => {
    ...
    const refresh = view === 'evaluation' ? api.getEvaluation() : view === 'operations' ? api.listFailures() : Promise.resolve(null)
    void refresh.then((value) => { if (value && 'providerUsage' in value) setEvaluation(value); else if (value) setFailures(value) }).catch((cause: unknown) => setError(cause instanceof Error ? cause.message : 'Could not refresh the workspace.'))
  }, [api, sessionOpen, view])

  async function openWorkspace(passcode: string) {
    setBusy(true); setError(null)
    try { const session = await api.createSession(passcode); api.setToken(session.token); await api.resetDemo(); const [page, currentEvaluation, currentFailures] = await Promise.all([api.listTickets(), api.getEvaluation(), api.listFailures()]); setTickets(page.content.map(mapApiTicket)); setEvaluation(currentEvaluation); setFailures(currentFailures); setSelectedTicketId(null); setView('inbox'); setSessionOpen(true) }
  ...
  async function hydrateTriage(ticket: Ticket) {
    ...
    try { await api.requestTriage(ticket.backendId); for (let attempt = 0; attempt < 40; attempt += 1) { const processing = await api.getProcessing(ticket.backendId); if (processing.result) { setTickets((current) => current.map((item) => item.backendId === ticket.backendId ? applyApiTriage(item, processing.result as TriageResult, processing.job) : item)); return } if (processing.job.status === 'RETRYABLE_FAILURE' || processing.job.status === 'TERMINAL_FAILURE') throw new Error(`Triage stopped with ${processing.job.status.toLowerCase().replaceAll('_', ' ')}.`); await new Promise((resolve) => window.setTimeout(resolve, 250)) } throw new Error('Triage did not complete within the local review window.') }
  ...
  async function retryFailure() { const failure = failures?.content[0]; if (!failure) return; setBusy(true); setError(null); try { await api.retryFailure(failure.id); setFailures(await api.listFailures()) } catch (cause: unknown) { setError(cause instanceof Error ? cause.message : 'Could not retry the failed case.') } finally { setBusy(false) } }

  if (!sessionOpen) return <Landing onOpen={openWorkspace} busy={busy} error={error} />
  return <div className="app-shell"><aside className="app-rail"><div className="rail-brand"><span className="brand-mark">CL</span><span>CaseLens</span></div><div className="rail-workspace"><span className="eyebrow">Workspace</span><strong>Isolated demo workspace</strong><span className="workspace-id">token-scoped / 24 hours</span></div><nav className="primary-nav" aria-label="Primary navigation"><button aria-label="Inbox" className={view === 'inbox' ? 'nav-item active' : 'nav-item'} onClick={() => setView('inbox')} aria-current={view === 'inbox' ? 'page' : undefined}><span className="nav-icon">↗</span><span>Inbox</span><span className="nav-count">{tickets.length}</span></button><button aria-label="Evaluation" className={view === 'evaluation' ? 'nav-item active' : 'nav-item'} onClick={() => setView('evaluation')} aria-current={view === 'evaluation' ? 'page' : undefined}><span className="nav-icon">◌</span><span>Evaluation</span></button><button aria-label="Operations" className={view === 'operations' ? 'nav-item active' : 'nav-item'} onClick={() => setView('operations')} aria-current={view === 'operations' ? 'page' : undefined}><span className="nav-icon">⌁</span><span>Operations</span><span className="nav-status">{failures?.totalElements ? failures.totalElements : 'OK'}</span></button></nav><div className="rail-footer"><div className="preview-label"><span className="status-dot" />Local API mode</div><p>All records are synthetic and scoped to this temporary workspace.</p><button className="text-button" onClick={() => setSessionOpen(false)}>Close demo</button></div></aside><main className="workspace-main"><header className="topbar"><div className="breadcrumbs"><span>CaseLens</span><span className="breadcrumb-slash">/</span><strong>{view === 'inbox' ? 'Review queue' : view === 'evaluation' ? 'Evaluation signal' : 'Operations'}</strong></div><div className="topbar-actions"><span className="live-chip"><span className="status-dot" />Backend data</span><span className="avatar">HV</span></div></header>{error && <p className="error-banner workspace-error" role="alert">{error}</p>}{view === 'inbox' && <InboxView tickets={tickets} selectedTicket={selectedTicket} draftUrgency={draftUrgency} feedbackNote={feedbackNote} feedbackSaved={feedbackSaved} busy={busy || Boolean(busyTicketId)} onLoadScenario={loadScenario} onSelectTicket={selectTicket} onBack={() => setSelectedTicketId(null)} onDraftUrgency={setDraftUrgency} onFeedbackNote={setFeedbackNote} onSaveFeedback={saveFeedback} />}{view === 'evaluation' && <EvaluationView evaluation={evaluation} />}{view === 'operations' && <OperationsView failures={failures} busy={busy} onRetry={retryFailure} />}</main></div>
}
...
function EvaluationView({ evaluation }: { evaluation: EvaluationSummary | null }) {
  const provider = evaluation?.providerUsage?.[0]
  const percent = (value: number | undefined) => value === undefined ? '—' : `${Math.round(value * 100)}%`
  return <div className="page-content narrow-page"><div className="page-heading"><div><p className="eyebrow">Measured quality</p><h1>Evaluation signal</h1><p className="page-intro">Metrics read from persisted triage results and human feedback in this workspace.</p></div><span className="date-stamp">{evaluation ? new Date(evaluation.generatedAt).toLocaleDateString() : 'Loading…'}</span></div><section className="signal-hero"><div><span className="eyebrow">Current signal</span><h2>Reviewers can see why the queue moved.</h2><p>Evaluation keeps model usefulness separate from human authority. Empty values mean this workspace has not processed a case yet.</p></div><div className="signal-score"><strong>{percent(evaluation?.agreementRate)}</strong><span>agreement</span></div></section><div className="metric-grid"><MetricCard label="Agreement" value={percent(evaluation?.agreementRate)} detail={`${evaluation?.evaluatedResults ?? 0} evaluated cases`} tone="teal" /><MetricCard label="Correction rate" value={percent(evaluation?.correctionRate)} detail={`${evaluation?.triageResults ?? 0} persisted results`} tone="coral" /><MetricCard label="p50 latency" value={evaluation ? `${evaluation.medianLatencyMs} ms` : '—'} detail={evaluation ? `p95 ${evaluation.p95LatencyMs} ms` : 'Waiting for data'} tone="ink" /><MetricCard label="Failure rate" value={percent(evaluation?.failureRate)} detail="From recorded triage jobs" tone={evaluation?.failureRate ? 'coral' : 'teal'} /></div><section className="provider-card"><div><span className="eyebrow">Provider usage</span><h2>{provider ? provider.provider : 'No provider calls yet'}</h2><p className="muted">Usage is read from model-invocation records without exposing credentials or raw ticket content.</p></div><div className="provider-row"><span>{provider ? `${provider.requests} requests / ${provider.successes} successes` : 'Deterministic mock provider'}</span><strong>{provider ? `${provider.inputTokens + provider.outputTokens} tokens` : '0'}</strong><span className="bar"><span style={{ width: provider ? '100%' : '0%' }} /></span></div></section></div>
}
...
function OperationsView({ failures, busy, onRetry }: { failures: FailurePage | null; busy: boolean; onRetry: () => void }) {
  const count = failures?.totalElements ?? 0
  const failure = failures?.content[0]
  return <div className="page-content narrow-page"><div className="page-heading"><div><p className="eyebrow">Reliability view</p><h1>Operations</h1><p className="page-intro">Queue failures and recovery actions read from the local backend.</p></div><span className="live-chip"><span className="status-dot" />{count ? 'Attention' : 'Healthy'}</span></div><section className={count ? 'operations-hero failure-state' : 'operations-hero'}><div className="checkmark">{count ? '!' : '✓'}</div><div><h2>{count ? `${count} unresolved failure${count === 1 ? '' : 's'}` : 'No unresolved failures'}</h2><p>{count ? `Job ${failure?.id} remains visible until a reviewer retries it.` : 'The local queue has no recorded retryable or terminal failures.'}</p></div>{count ? <button className="primary-button" onClick={() => void onRetry()} disabled={busy}>Retry failed case</button> : <span className="muted">Observed from backend</span>}</section><div className="operations-grid"><section className="ops-card"><span className="eyebrow">Queue state</span><strong>{count ? 'Attention' : 'Clear'}</strong><p>Failures remain visible until recovery.</p></section><section className="ops-card"><span className="eyebrow">Review backlog</span><strong>{count}</strong><p>Operational cases requiring action.</p></section><section className="ops-card"><span className="eyebrow">Provider health</span><strong>{count ? 'Degraded' : 'Nominal'}</strong><p>Derived from persisted job state.</p></section></div><section className="runbook-card"><span className="eyebrow">Failure handling</span><h2>Every failure has a next state.</h2><div className="runbook-steps"><div><span>01</span><strong>Retryable</strong><p>Keep the case queued and visible.</p></div><div><span>02</span><strong>Terminal</strong><p>Record the failure and surface it.</p></div><div><span>03</span><strong>Human review</strong><p>Make the decision safe to explain.</p></div></div></section></div>
}
