...
| Area | Evidence |
| --- | --- |
| Backend | 115 tests passed; 0 failures, errors, or skips; PostgreSQL and LocalStack Testcontainers included |
...
| Terraform | `fmt -check`, provider initialization with the lockfile, application and bootstrap `terraform validate`, read-only base/compute plans, the SnapStart alias tests (3 passed: routing, fresh bootstrap, and required later-release pins), and bootstrap state-bucket/destroy-role plan passed; destroy workflow YAML and safety guards parsed locally |
...
| Custom domain | On 2026-07-29, `https://caselens.harishdarko.com` and the CloudFront fallback both returned HTTP 200 over TLS. Cloudflare is DNS-only in front of the existing distribution, ACM uses `us-east-1`, and API CORS allowed the custom origin. |
| Recovery cadence | On 2026-07-29, the EventBridge recovery sweep was changed from one minute to 30 minutes. Immediate outbox publication remains the normal path. The old cadence prevented Neon from idling for five minutes and exhausted its free compute allowance; visitor traffic and storage growth were not the cause. |
...
| Worker latency | SQS invokes pinned SnapStart worker version 6; a cold synthetic Groq journey completed in 7.27 s versus 40.46 s before constructor-time bootstrap, and the next journey completed in 2.69 s; both were `AI_VALIDATED` in one attempt |
...
Live reviewer URL: [https://caselens.harishdarko.com](https://caselens.harishdarko.com).
CloudFront fallback: [https://d27d60ya5pvyzq.cloudfront.net](https://d27d60ya5pvyzq.cloudfront.net).
